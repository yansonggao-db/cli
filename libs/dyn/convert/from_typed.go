...
func fromTypedStruct(src reflect.Value, ref dyn.Value, options ...fromTypedOptions) (dyn.Value, error) {
	...
	for _, fieldval := range info.FieldValues(src) {
		...
		// If the field carries the bundle:"sensitive" tag and resolved to a
		// non-empty string, wrap it as a sensitive value so that all downstream
		// serializers (JSON, YAML) redact it automatically.
		if info.Sensitive[k] && nv.Kind() == dyn.KindString {
			if s, ok := nv.AsString(); ok && s != "" {
				nv = dyn.NewSensitiveValue(s, nv.Locations())
			}
		}
